...
local playerExpInfo = nil

-- private functions
local function addButton(id, description, icon, callback, panel, toggle, front, index, tenlilUI)
  local class
  if toggle then
...
function init()
  ...
  playerInfo = topMenu:recursiveGetChildById('playerInfo')
  playerExpInfo = topMenu:recursiveGetChildById('playerExpInfo')
...
function setPlayerInfo(name, level, vocation)
  playerInfo:setText(
    name .. ' - Level ' .. level .. ' ' .. vocation
  )
  playerInfo:setVisible(true)
end

function setPlayerExpInfo(exph, estimate)
  local expText = exph .. ' exp. per hour'
  if(estimate) then
    expText = expText .. ' - Level up in approx. ' .. estimate .. '.'
  end
  playerExpInfo:setText(
    expText
  )
  playerExpInfo:setVisible(true)
end
...
function offline()
  ...
  playerExpInfo:setVisible(false)
  if topMenu.hideIngame then
    show()
  end
  if topMenu.onlineLabel then
...
function addRightToggleButton(id, description, icon, callback, front, index, tenlilUI)
  return addButton(id, description, icon, callback, topMenu.rightButtonsPanel, true, front, index, tenlilUI)
end
...
function addRightGameToggleButton(id, description, icon, callback, front, index, tenlilUI)
  local button = addButton(id, description, icon, callback, topMenu.rightGameButtonsPanel, true, front, index, tenlilUI)
  if modules.game_buttons then
    modules.game_buttons.takeButton(button)
  end
  return button
...
function showGameButtons()
  ...
  topMenu.rightButtonsPanel:setOn(false)
end

function hideGameButtons()
  topMenu.rightButtonsPanel:setOn(true)
...
function show()
  topMenu:setHeight(topMenuCompat.initialHeight)
  topMenu:show()
  if not topMenu.hideIngame then
    modules.game_interface.getRootPanel():addAnchor(AnchorTop, 'topMenu', AnchorBottom)
  end
