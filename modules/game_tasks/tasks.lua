...
local taskIcons = {
    ...
    warlock = "/images/game/creatures/monsters/warlock",
    hero = "/images/game/creatures/monsters/hero",
    hydra = "/images/game/creatures/monsters/hydra",
    demon = "/images/game/creatures/monsters/demon",
    behemoth = "/images/game/creatures/monsters/behemoth",
    ...
    ["serpent spawn"] = "/images/game/creatures/monsters/serpent_spawn",
    ["ancient scarab"] = "/images/game/creatures/monsters/ancient_scarab",
    ["black knight"] = "/images/game/creatures/monsters/black_knight",
...
function init()
    ...
    if ProtocolGame and ProtocolGame.registerExtendedOpcode then
        ...
        ProtocolGame.registerExtendedOpcode(ExtendedIds.TaskData,
                                            onExtendedTaskData)
        ProtocolGame.registerExtendedOpcode(ExtendedIds.TaskUpdate,
                                            onExtendedTaskUpdate)
    else
...
function onExtendedTaskData(protocol, opcode, buffer)
    ...
    if data and type(data) == 'table' then onTaskData(data) end
end

function onExtendedTaskUpdate(protocol, opcode, buffer)
    local data = json.decode(buffer)
    if data and type(data) == 'table' then onTaskUpdate(data) end
end
...
function requestTasks()
    ...
    if g_game.getFeature(GameExtendedClientPing) then
        ...
        displayErrorWindow(
            "Extended protocol feature is not available. Tasks cannot be loaded.")
    end
end
...
function onTaskData(data)
    ...
    g_logger.debug("Structure of availableTasks: " ..
                       json.encode(data.availableTasks or {}))
    g_logger.debug("Structure of currentTasks: " ..
                       json.encode(data.currentTasks or {}))
    ...
    if data.currentTasks and data.currentTasks.daily then
        for _, task in ipairs(data.currentTasks.daily) do
            g_logger.info("DAILY TASK " .. task.id .. " (" .. task.name ..
                              ") DETAILS: " .. json.encode(task))

            if task.reward then
                g_logger.info("DAILY TASK " .. task.id .. " REWARDS: " ..
                                  json.encode(task.reward))

                if task.reward.items then
                    g_logger.info(
                        "DAILY TASK " .. task.id .. " ITEMS COUNT: " ..
                            #task.reward.items)
                    for i, item in ipairs(task.reward.items) do
                        g_logger.info(
                            "DAILY TASK " .. task.id .. " ITEM " .. i .. ": " ..
                                json.encode(item))
                    end
                else
                    g_logger.info("DAILY TASK " .. task.id ..
                                      " HAS NO ITEM REWARDS DEFINED!")
                end
            else
                g_logger.info("DAILY TASK " .. task.id ..
                                  " HAS NO REWARDS STRUCTURE!")
            end
        end
    end
    ...
    if data.currentTasks and data.currentTasks.normal then
        for _, task in ipairs(data.currentTasks.normal) do
            g_logger.info("NORMAL TASK " .. task.id .. " (" .. task.name ..
                              ") DETAILS: " .. json.encode(task))

            if task.reward then
                g_logger.info("NORMAL TASK " .. task.id .. " REWARDS: " ..
                                  json.encode(task.reward))

                if task.reward.items then
                    g_logger.info(
                        "NORMAL TASK " .. task.id .. " ITEMS COUNT: " ..
                            #task.reward.items)
                    for i, item in ipairs(task.reward.items) do
                        g_logger.info(
                            "NORMAL TASK " .. task.id .. " ITEM " .. i .. ": " ..
                                json.encode(item))
                    end
                else
                    g_logger.info("NORMAL TASK " .. task.id ..
                                      " HAS NO ITEM REWARDS DEFINED!")
                end
            else
                g_logger.info("NORMAL TASK " .. task.id ..
        ...
        for _, task in ipairs(data.currentTasks.daily) do
            if task.id == 102 then
                ...
                if task.monsters then
                    g_logger.info("MINOTAUR MONSTERS: " ..
                                      json.encode(task.monsters))
                else
                ...
                if task.reward and task.reward.items then
                    g_logger.info("MINOTAUR TASK ITEMS: " ..
                                      json.encode(task.reward.items))
                else
            ...
            if task.id == 105 then
                ...
                if task.monsters then
                    g_logger.info("DRAGON LAIR MONSTERS: " ..
                                      json.encode(task.monsters))
                else
                ...
                -- Log item rewards specifically
                if task.reward and task.reward.items then
                    g_logger.info("DRAGON LAIR TASK ITEMS: " ..
                                      json.encode(task.reward.items))

                    -- Verificar cada item individualmente
                    for i, item in ipairs(task.reward.items) do
                        g_logger.info("DRAGON LAIR ITEM " .. i .. ": " ..
                                          json.encode(item))
                    end
                else
                    g_logger.info(
                        "DRAGON LAIR TASK HAS NO ITEM REWARDS DEFINED!")
                end
            end
        end
    end
    ...
    if data.availableTasks and data.availableTasks.daily then
        for _, task in ipairs(data.availableTasks.daily) do
            if task.id == 102 then
                g_logger.info("AVAILABLE MINOTAUR TASK DETAILS: " ..
                                  json.encode(task))
                if task.monsters then
                    g_logger.info("AVAILABLE MINOTAUR MONSTERS: " ..
                                      json.encode(task.monsters))
                else
                    g_logger.info(
                        "AVAILABLE MINOTAUR TASK HAS NO MONSTERS DEFINED!")
                end

                -- Log item rewards specifically
                if task.reward and task.reward.items then
                    g_logger.info("AVAILABLE MINOTAUR TASK ITEMS: " ..
                                      json.encode(task.reward.items))
                else
                    g_logger.info(
                        "AVAILABLE MINOTAUR TASK HAS NO ITEM REWARDS DEFINED!")
                end
            end
        end
    end
        ...
        for _, task in ipairs(taskList) do
            ...
            if not task.id then
                ...
                if task.monsters and #task.monsters > 0 then
                    g_logger.debug("Task " .. task.id ..
                                       " already has monsters defined: " ..
                                       table.concat(task.monsters, ", "))
                else
                    g_logger.debug("Task " .. task.id ..
                                       " has no monsters defined, will try to determine them")
                end

                -- Log item rewards received from the server
                if task.reward and task.reward.items then
                    g_logger.debug(
                        "Task " .. task.id .. " has item rewards: " ..
                            json.encode(task.reward.items))
                else
                    g_logger.debug("Task " .. task.id ..
                                       " has no item rewards, will use defaults if needed")
                end
                ...
                if not task.monsters or #task.monsters == 0 then
                    ...
                    g_logger.debug("Derived monsters for task " .. task.id ..
                                       ": " .. table.concat(task.monsters, ", "))
                end

                -- Ensure reward information exists
                if not task.reward then task.reward = {} end
                ...
                if task.reward.items then
                    ...
                    for i, item in ipairs(task.reward.items) do
                        ...
                        if type(item) ~= "table" then
                            g_logger.error(
                                "Item " .. i .. " for task " .. task.id ..
                                    " is not a table: " .. tostring(item))
                        else
                    ...
                    g_logger.debug(
                        "Final processed items for task " .. task.id .. ": " ..
                            json.encode(task.reward.items))
                end
                ...
                if not task.type then task.type = taskType end
    ...
    local function validateTaskRewards(taskList)
        ...
        for _, task in ipairs(taskList) do
            ...
            task.reward.points = task.reward.points or
                                     (task.type == TASK_TYPE_DAILY and 1 or 2)
            task.reward.teleport = task.reward.teleport or "None"

            -- Ensure items array exists
            if not task.reward.items then task.reward.items = {} end
            ...
            g_logger.debug("Validated rewards for task " .. task.id .. ": " ..
                               json.encode(task.reward))
        end
    end
    ...
    g_logger.info("Task counts - Available Normal: " .. normalCount ..
                      ", Available Daily: " .. dailyCount)
    ...
    g_logger.info("Task counts - Current Normal: " .. currentNormalCount ..
                      ", Current Daily: " .. currentDailyCount)
    ...
    local function ensureCompleteRewards(task)
        ...
        for i, item in ipairs(task.reward.items) do
            ...
            if type(item) ~= "table" then
                g_logger.error("Item " .. i .. " for task " .. task.id ..
                                   " is not a table: " .. tostring(item))
            else
                ...
                g_logger.debug("Ensured item " .. i .. " structure for task " ..
                                   task.id .. ": " .. json.encode(item))
            end
        end
            ...
            for _, currentTask in ipairs(currentTasks.normal) do
                if currentTask.id == task.id then
                    if currentTask.status == TASK_STATUS_STARTED then
                        isInProgress = true
                        g_logger.debug(
                            "Tarefa em andamento encontrada em currentTasks.normal")
                        break
                    end
                end
            end
            ...
            for _, currentTask in ipairs(currentTasks.daily) do
                if currentTask.id == task.id then
                    if currentTask.status == TASK_STATUS_STARTED then
                        isInProgress = true
                        g_logger.debug(
                            "Tarefa em andamento encontrada em currentTasks.daily")
                        break
                    end
                end
            end
...
function onTaskUpdate(data)
    ...
    if data.taskId and data.count ~= nil then
        g_logger.debug("Received task update for ID " .. data.taskId ..
        ...
        if currentTasks.normal then
            for i, task in ipairs(currentTasks.normal) do
                if task.id == data.taskId then
                    ...
                    updatedTask = task
                    g_logger.debug("Updated normal task: " .. task.name ..
                                       " with count " .. data.count)
                    break
                end
            end
        ...
        if not updatedTask and currentTasks.daily then
            for i, task in ipairs(currentTasks.daily) do
                if task.id == data.taskId then
                    task.count = data.count
                    updatedTask = task
                    g_logger.debug("Updated daily task: " .. task.name ..
                                       " with count " .. data.count)
                    break
                end
            end
...
function displayTasksWindow(taskPoints)
    ...
    local totalTaskPointsLabel = tasksWindow:recursiveGetChildById(
                                     'totalTaskPoints')
    ...
    local children = {}
    if taskList then children = taskList:getChildren() end
    ...
    if children and #children > 0 then
        ...
        if firstTaskWidget then
            g_logger.info("Selecting initial task: " ..
                              (firstTaskWidget.task.name or "Unknown"))
            ...
            if firstTaskWidget.task.type == TASK_TYPE_DAILY and
                currentTasks.daily then
                for _, task in ipairs(currentTasks.daily) do
                    if task.id == taskId then
                        completeTask = task
                        break
            ...
            if not completeTask and firstTaskWidget.task.type == TASK_TYPE_DAILY and
                availableTasks.daily then
                for _, task in ipairs(availableTasks.daily) do
                    if task.id == taskId then
                        completeTask = task
                        break
            ...
            if completeTask then selectedTask = completeTask end
            ...
            scheduleEvent(function()
                ...
                if tasksWindow and tasksWindow:isVisible() and selectedTask then
                    g_logger.debug(
                        "Refreshing task details after selection for: " ..
                            selectedTask.id)

                    -- Ensure the reward structure exists
                    if not selectedTask.reward then
                        selectedTask.reward = {
                            points = (selectedTask.type == TASK_TYPE_DAILY and 1 or
                                2),
                        ...
                        g_logger.debug(
                            "Created default reward structure for task: " ..
                                selectedTask.id)
                    end
...
function filterTasks()
    ...
    if scrollBar and type(scrollBar.setRange) == "function" then
        scrollBar:setRange(0, math.max(0,
                                       visibleCount * 30 - taskList:getHeight()))
    end
end
...
function populateTaskList()
    ...
    local function isTaskInCurrentTasks(taskId)
        ...
        if currentTasks.daily then
            for _, task in ipairs(currentTasks.daily) do
                if task.id == taskId then return true end
            end
        end
    ...
    if currentTasks and currentTasks.daily and #currentTasks.daily > 0 then
        ...
        for i, task in ipairs(currentTasks.daily) do
            g_logger.info("Adding daily task " .. i .. ": " ..
                              (task.name or "Unknown"))
    ...
    if availableDailyTasksCount > 0 then
        ...
        for i, task in ipairs(availableTasks.daily) do
            if not isTaskInCurrentTasks(task.id) then
                g_logger.info("Adding available daily task: " ..
                                  (task.name or "Unknown"))
                local widget = addTaskToList(task, taskList, itemCount % 2 == 0)
                itemCount = itemCount + 1
            end
    ...
    if currentTasks and currentTasks.normal and #currentTasks.normal > 0 then
        ...
        for i, task in ipairs(currentTasks.normal) do
            g_logger.info("Adding current normal task " .. i .. ": " ..
                              (task.name or "Unknown"))
            local widget = addTaskToList(task, taskList, itemCount % 2 == 0)
            itemCount = itemCount + 1
        end
    ...
    if availableNormalTasksCount > 0 then
        ...
        for i, task in ipairs(availableTasks.normal) do
            if not isTaskInCurrentTasks(task.id) then
                g_logger.info("Adding available normal task: " ..
                                  (task.name or "Unknown"))
                local widget = addTaskToList(task, taskList, itemCount % 2 == 0)
                itemCount = itemCount + 1
            end
    ...
    for _, child in pairs(children) do
        if child.task then
            child.onClick = function()
                ...
                g_logger.info("Task selected: " ..
                                  (child.task.name or "unknown") .. " (ID: " ..
                                  tostring(child.task.id) .. ")")
                ...
                for _, otherChild in pairs(children) do
                    if otherChild.task then
                        otherChild:setBackgroundColor(
                            otherChild == child and '#444444' or 'alpha')

                        -- Find the nameLabel inside the widget
                        local nameLabel = otherChild:getChildById('taskName')
                        if nameLabel then
                            nameLabel:setColor(
                                otherChild == child and '#ffffff' or '#cccccc')

                            -- If it's a daily task, preserve its color
                            if otherChild.task.type == TASK_TYPE_DAILY and
                                otherChild ~= child then
                                nameLabel:setColor('#ffcc00')
                            end

                            -- If it's a recommended task, preserve its color
                            if otherChild.task.recommended and otherChild ~=
                                child then
                ...
                if selectedTask.type == TASK_TYPE_DAILY and currentTasks.daily then
                    for _, task in ipairs(currentTasks.daily) do
                        if task.id == selectedTask.id then
                            completeTask = task
                            g_logger.debug(
                                "Found complete task info in currentTasks.daily")
                            break
                        end
                    end
                elseif currentTasks.normal then
                    for _, task in ipairs(currentTasks.normal) do
                        if task.id == selectedTask.id then
                            completeTask = task
                            g_logger.debug(
                                "Found complete task info in currentTasks.normal")
                            break
                        end
                    end
                end
                ...
                if not completeTask and selectedTask.type == TASK_TYPE_DAILY and
                    availableTasks.daily then
                    for _, task in ipairs(availableTasks.daily) do
                        if task.id == selectedTask.id then
                            completeTask = task
                            g_logger.debug(
                                "Found complete task info in availableTasks.daily")
                            break
                        end
                    end
                elseif not completeTask and availableTasks.normal then
                    for _, task in ipairs(availableTasks.normal) do
                        if task.id == selectedTask.id then
                            completeTask = task
                            g_logger.debug(
                                "Found complete task info in availableTasks.normal")
                            break
                        end
                    end
                end
                ...
                scheduleEvent(function()
                    ...
                    if tasksWindow and tasksWindow:isVisible() and selectedTask then
                        ...
                        if not selectedTask.reward then
                            selectedTask.reward = {
                                points = (selectedTask.type == TASK_TYPE_DAILY and
                                    1 or 2),
                                exp = ((selectedTask.level or 1) * 100),
                                gold = ((selectedTask.level or 1) * 50),
                                items = {},
...
function getMonsterTypeFromName(taskName)
    ...
    if taskToMonster[lowerName] then
        g_logger.debug("Found exact match for task: " .. lowerName .. " -> " ..
                           taskToMonster[lowerName])
    ...
    for keyword, monsterType in pairs(keywords) do
        if lowerName:find(keyword) then
            g_logger.debug(
                "Found keyword in task name: " .. keyword .. " in " .. lowerName)
            return monsterType
        end
    end

    g_logger.debug("No monster type found for task name: " .. lowerName ..
                       ", using default")
    return "default"
end
...
function addTaskToList(task, container, isEven)
    ...
    if not iconPath then
        ...
        if baseType then
            g_logger.debug("Extracted base monster type: " .. baseType ..
                               " from " .. monsterType)
    ...
    if not iconPath then
        g_logger.debug("No icon found for monster: " .. monsterType ..
                           ", using default")
    ...
    if iconWidget then iconWidget:setImageSource(iconPath) end
    ...
    if levelLabel and task.level then
        ...
        if levelLabel then levelLabel:setVisible(false) end
    end
    ...
    elseif taskStatus == "in_progress" and task.count and task.count > 0 and
        nameLabel then
        nameLabel:setText(taskNameText .. " [" .. task.count .. "/" ..
                              (task.total or 100) .. "]")
    end
...
function updateTaskDetails(task)
    ...
    g_logger.debug("Updating details for task: " .. task.name .. " (ID: " ..
                       task.id .. ")")
    ...
    g_logger.debug("Exp field in task.reward: " .. type(task.reward.exp) ..
                       " = " .. tostring(task.reward.exp))
    ...
    local monstersDisplayPanel = tasksWindow:recursiveGetChildById(
                                     'monstersDisplayPanel')
    ...
    local experienceLabel =
        rewardsPanel:recursiveGetChildById('experienceLabel')
    local goldLabel = rewardsPanel:recursiveGetChildById('goldLabel')
    local taskPointsLabel =
        rewardsPanel:recursiveGetChildById('taskPointsLabel')
    ...
    local function setupLabel(label, text)
        if label then
            ...
            if string.len(text) > 40 then label:setTextWrap(true) end
        else
    ...
    if task.reward and task.reward.items and #task.reward.items > 0 then
        ...
        for i, item in ipairs(task.reward.items) do
            ...
            local displayName = item.name or
                                    ("Item #" .. (item.id or "unknown"))
            local count = item.count or 1

            -- Formatar nome do item (capitalizar palavras)
            displayName = displayName:gsub("(%a)([%w_']*)",
                                           function(first, rest)
                return first:upper() .. rest:lower()
            end)

            -- Format the item text for display
            if i > 1 then itemText = itemText .. ",\n" end
    ...
    local function setupMonsterPanel(panel, monsterName, visible)
        ...
        local nameLabel =
            panel:getChildById(panel:getId():gsub('Panel', 'Name'))
    ...
    if monstersList and #monstersList > 0 then
        g_logger.info("Setting up monsters for task: " .. task.id ..
                          ", Monster count: " .. #monstersList)
        ...
        g_logger.info("No monsters defined for task: " .. task.id ..
                          ", using default")
    ...
    if killsRequired then killsRequired:setText(tostring(requiredKills)) end
    ...
    if taskStatusLabel then
        ...
        if task.type == TASK_TYPE_NORMAL then
            ...
            if currentTasks.normal then
                for _, currentTask in ipairs(currentTasks.normal) do
                    if currentTask.id == task.id and currentTask.status ==
                        TASK_STATUS_COMPLETED then
                        isCompleted = true
                        break
                    end
                end
            ...
            if task.timesDone and task.maxRepeats then
                statusText = "Daily task - " .. task.timesDone .. "/" ..
                                 task.maxRepeats .. " done today"
                shouldShow = true
    ...
    if killsProgress then
        ...
        local progressPercent = math.min(100, math.floor(
                                             (currentCount / requiredKills) *
                                                 100))
        g_logger.debug("Setting progress to " .. progressPercent .. "% (" ..
                           currentCount .. "/" .. requiredKills .. ")")
    ...
    if startTaskButton then
        ...
        if task.type == TASK_TYPE_DAILY and task.timesDone and task.maxRepeats then
            if task.timesDone >= task.maxRepeats then
                cannotStart = true
                reasonText = "Max Daily Limit (" .. task.timesDone .. "/" ..
                                 task.maxRepeats .. ")"
            end
        end
            ...
            for _, currentTask in ipairs(currentTasks.normal) do
                if currentTask.id == task.id then
                    if currentTask.status == TASK_STATUS_STARTED then
                        isInProgress = true
                        g_logger.debug(
                            "Task found in progress in currentTasks.normal")
                    elseif currentTask.status == TASK_STATUS_COMPLETED and
                        task.type == TASK_TYPE_NORMAL then
                        isCompleted = true
                        cannotStart = true
                        reasonText = "Already Completed"
                        g_logger.debug(
                            "Normal task found completed in currentTasks.normal")
                    end
                    break
                end
            end
            ...
            for _, currentTask in ipairs(currentTasks.daily) do
                if currentTask.id == task.id then
                    if currentTask.status == TASK_STATUS_STARTED then
                        isInProgress = true
                        g_logger.debug(
                            "Task found in progress in currentTasks.daily")
                    end
                    break
                end
            end
...
function startTask()
    ...
    -- Check if task status is STARTED
    if selectedTask.status == TASK_STATUS_STARTED then isInProgress = true end
    ...
    if g_game.getFeature(GameExtendedClientPing) then
        ...
        if protocol then
            ...
            local data = {action = action, taskId = selectedTask.id}
            protocol:sendExtendedOpcode(ExtendedIds.TaskAction,
                                        json.encode(data))
