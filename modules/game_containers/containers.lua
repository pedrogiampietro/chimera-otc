...
local function isAutoClosableLoot(container)
    ...
    local isCorpseByName = (name:find('dead') ~= nil) or
                               (name:find('remains') ~= nil) or
                               (name:find('slain') ~= nil) or
                               (name:find('corpse') ~= nil)
    ...
    if citem and citem.isPickupable and not citem:isPickupable() and
        (not container:hasParent()) then isLikelyGroundLoot = true end

    -- Conversely, avoid closing known personal/storage containers by name
    local isPersonalContainerByName = (name:find('backpack') ~= nil) or
                                          (name:find('bag') ~= nil) or
                                          (name:find('chest') ~= nil) or
                                          (name:find('box') ~= nil) or
                                          (name:find('crate') ~= nil) or
                                          (name:find('locker') ~= nil) or
                                          (name:find('depot') ~= nil) or
                                          (name:find('stash') ~= nil)

    -- Auto-close only monster loot/corpse-like containers, and never personal storage
    return (isCorpseByName or isLikelyGroundLoot) and
               not isPersonalContainerByName
end
...
function refreshContainerItems(container)
    if not container.itemsPanel then return end
...
function toggleContainerPages(containerWindow, hasPages)
    if hasPages == containerWindow.pagePanel:isOn() then return end

    containerWindow.pagePanel:setOn(hasPages)

    if hasPages then
        containerWindow.miniwindowScrollBar:setMarginTop(
            containerWindow.miniwindowScrollBar:getMarginTop() +
                containerWindow.pagePanel:getHeight())
        containerWindow.contentsPanel:setMarginTop(
            containerWindow.contentsPanel:getMarginTop() +
                containerWindow.pagePanel:getHeight())
    else
        containerWindow.miniwindowScrollBar:setMarginTop(
            containerWindow.miniwindowScrollBar:getMarginTop() -
...
function onContainerOpen(container, previousContainer)
    ...
    if previousContainer then
        ...
        if containerPanel and containerPanel:getClassName() ==
            'UIMiniWindowContainer' then
            local openContainers = {}
            for _, cont in pairs(g_game.getContainers()) do
                if cont.window and cont.window:isVisible() and
                    cont.window:getParent() == containerPanel then
                    -- Only consider auto-closable loot containers here (skip backpacks and personal storage)
                    if isAutoClosableLoot(cont) then
                        table.insert(openContainers, {
                            container = cont,
                            yPos = cont.window:getY()
                        })
                    end
                end
            end
            ...
            table.sort(openContainers, function(a, b)
            ...
            if #openContainers > 0 then
                ...
                if lastContainer.container.window then
                    estimatedNextY = lastContainer.container.window:getY() +
                                         lastContainer.container.window:getHeight() +
                                         5
                end
            end

            -- Close oldest container if we exceed the limit OR if running out of screen space
            if #openContainers >= MAX_VISIBLE_CONTAINERS or estimatedNextY >
                (panelHeight - 150) then
                local oldestContainer = openContainers[1]
                if oldestContainer and oldestContainer.container and
                    oldestContainer.container.window then
        ...
        scheduleEvent(function()
            if containerWindow then containerWindow:setBorderWidth(0) end
        end, 300)
    end
    ...
    function containerWindow.onDrop(container, widget, mousePos)
        if containerPanel:getChildByPos(mousePos) then return false end

        local child = containerPanel:getChildByIndex(-1)

        if child then child:onDrop(widget, mousePos, true) end
    end
    ...
    function upButton.onClick() g_game.openParent(container) end
    ...
    if container:hasPages() then
        local height = containerWindow.miniwindowScrollBar:getMarginTop() +
                           containerWindow.pagePanel:getHeight() + 17
