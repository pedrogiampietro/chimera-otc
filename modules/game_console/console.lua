SpeakTypesSettings = {
    ...
    channelYellow = {speakType = MessageModes.Channel, color = '#eec900'},
    channelWhite = {
...
SayModes = {
    [1] = {
        ...
        icon = '/images/ui/console/icons/sound_button_0'
    },
    [2] = {
        speakTypeDesc = 'say',
        icon = '/images/ui/console/icons/sound_button_1'
    },
    [3] = {
        speakTypeDesc = 'yell',
        icon = '/images/ui/console/icons/sound_button_2'
    }
}
...
function init()
    ...
    consolePanel.onKeyPress = function(self, keyCode, keyboardModifiers)
        ...
        local selection = tab.tabPanel:getChildById('consoleBuffer')
                              .selectionText
    ...
    g_keyboard.bindKeyPress('Shift+Up',
                            function() navigateMessageHistory(1) end,
                            consolePanel)
    g_keyboard.bindKeyPress('Shift+Down',
                            function() navigateMessageHistory(-1) end,
                            consolePanel)
    g_keyboard.bindKeyPress('Tab', function() consoleTabBar:selectNextTab() end,
                            consolePanel)
    g_keyboard.bindKeyPress('Shift+Tab',
                            function() consoleTabBar:selectPrevTab() end,
                            consolePanel)
    g_keyboard.bindKeyDown('Enter', sendCurrentMessage, consolePanel)
    g_keyboard.bindKeyPress('Ctrl+A',
                            function() consoleTextEdit:clearText() end,
                            consolePanel)
...
function enableChat(temporarily)
    ...
    consoleToggleChat:setTooltip(tr(
...
function load()
    ...
    if settings then messageHistory = settings.messageHistory or {} end
...
function onDragEnter(widget, pos) return floatingMode end

function onDragMove(widget, pos, moved)
    if not floatingMode then return end
    -- update margin
    return true
end

function onDragLeave(widget, pos) return floatingMode end

function clearChannel(consoleTabBar)
    consoleTabBar:getCurrentTab().tabPanel:getChildById('consoleBuffer')
        :destroyChildren()
end
...
function openPlayerReportRuleViolationWindow()
    ...
    violationWindow.onEnter = function()
        ...
        addTabText(tr('Please wait patiently for a gamemaster to reply') .. '.',
                   SpeakTypesSettings.privateRed, violationReportTab)
        addTabText(applyMessagePrefixies(g_game.getCharacterName(), 0, text),
                   SpeakTypesSettings.say, violationReportTab,
...
function addPrivateText(text, speaktype, name, isPrivateCommand, creatureName)
    ...
    if privateTab == nil then
        if (modules.client_options.getOption('showPrivateMessagesInConsole') and
            not focus) or (isPrivateCommand and not privateTab) then
...
local letterWidth =
    { -- New line (10) and Space (32) have width 1 because they are printed and not replaced with spacer
        [10] = 1,
        [32] = 1,
        [33] = 3,
        [34] = 6,
...
function addTabText(text, speaktype, tab, creatureName)
    ...
    label.onDragLeave = function(self, droppedWidget, mousePos)
        local text = {}
        for selectionChild = consoleBuffer.selection.first, consoleBuffer.selection
            .last do
    ...
    label.onDragMove = function(self, mousePos, mouseMoved)
            ...
            if mousePos.y < self:getY() then
                for index = selfIndex - 1, 1, -1 do
                    ...
                    if label:getY() + label:getHeight() > parentRect.y then
                        if (mousePos.y >= label:getY() and mousePos.y <=
                            label:getY() + label:getHeight()) or index == 1 then
                            child = label
                            break
                        end
                    else
            ...
            elseif mousePos.y > self:getY() + self:getHeight() then
                for index = selfIndex + 1, parent:getChildCount(), 1 do
                    ...
                    if label:getY() < parentRect.y + parentRect.height then
                        if (mousePos.y >= label:getY() and mousePos.y <=
                            label:getY() + label:getHeight()) or index ==
        ...
        if child ~= self then
            for selectionChild = consoleBuffer.selection.first + 1, consoleBuffer.selection
                .last - 1 do
...
function removeTabLabelByName(tab, name)
    ...
    local consoleBuffer = panel:getChildById('consoleBuffer')
    for _, label in pairs(consoleBuffer:getChildren()) do
        if label.name == name then label:destroy() end
    end
end
...
function processChannelTabMenu(tab, mousePos, mouseButton)
    ...
    if consoleTabBar:getCurrentTab() == tab then
        menu:addOption(tr('Clear Messages'),
                       function() clearChannel(consoleTabBar) end)
        menu:addOption(tr('Save Messages'), function()
            ...
            local filename = worldName .. ' - ' .. characterName .. ' - ' ..
                                 channelName .. '.txt'
            ...
            if g_resources.fileExists(filepath) then
                table.insert(lines, 1, protectedcall(
                                 g_resources.readFileContents, filepath) or '')
            end
...
function processMessageMenu(mousePos, mouseButton, creatureName, text, label,
    ...
    if mouseButton == MouseRightButton then
        ...
        if creatureName and #creatureName > 0 then
            ...
            if modules.game_ruleviolation.hasWindowAccess() then
                menu:addOption(tr('Rule Violation'), function()
                    modules.game_ruleviolation.show(creatureName,
                                                    text:match('.+%:%s(.+)'))
                end)
                menu:addSeparator()
            end
        ...
        if text then
            menu:addOption(tr('Copy message'),
        ...
        if tab.violations and creatureName then
            menu:addSeparator()
            menu:addOption(tr('Process') .. ' ' .. creatureName,
                           function()
...
function sendMessage(message, tab)
    ...
    local findIni, findEnd, chatCommandInitial, chatCommandPrivate,
          chatCommandEnd, chatCommandMessage = message:find(
                                                   "([%*%@])(.+)([%*%@])(.*)")
    ...
    currentMessageIndex = 0
    if #messageHistory == 0 or messageHistory[#messageHistory] ~=
        originalMessage then
    ...
    if (channel or tab == defaultTab) and not chatCommandPrivateReady then
        if tab == defaultTab then
            speaktypedesc = chatCommandSayMode or
                                SayModes[consolePanel:getChildById(
            ...
            g_game.talkChannel(SpeakTypesSettings[speaktypedesc].speakType,
                               channel, message)
            return
        end
    else
        ...
        if not dontAdd then
            message = applyMessagePrefixies(g_game.getCharacterName(),
                                            player:getLevel(), message)
            addPrivateText(message, speaktype, tabname, isPrivateCommand,
                           g_game.getCharacterName())
        end
    end
end
...
function sayModeChange(sayMode)
    ...
    buttom:setImageSource(SayModes[sayMode].icon)
...
function navigateMessageHistory(step)
    ...
    if numCommands > 0 then
        currentMessageIndex = math.min(math.max(currentMessageIndex + step, 0),
                                       numCommands)
        if currentMessageIndex > 0 then
            local command =
                messageHistory[numCommands - currentMessageIndex + 1]
...
function onTalk(name, level, mode, message, channelId, creaturePos)
    if mode == MessageModes.GamemasterBroadcast then
        modules.game_textmessage
            .displayBroadcastMessage(name .. ': ' .. message)
        return
    end

    local isNpcMode = (mode == MessageModes.NpcFromStartBlock or mode ==
                          MessageModes.NpcFrom)
    ...
    if name ~= g_game.getCharacterName() and isUsingIgnoreList() and
        not (isUsingWhiteList()) or
        (isUsingWhiteList() and not (isWhitelisted(name)) and
            not (isAllowingVIPs() and localPlayer:hasVip(name))) then
    ...
    if (mode == MessageModes.Say or mode == MessageModes.Whisper or mode ==
        MessageModes.Yell or mode == MessageModes.Spell or mode ==
        MessageModes.MonsterSay or mode == MessageModes.MonsterYell or mode ==
        MessageModes.NpcFrom or mode == MessageModes.BarkLow or mode ==
        MessageModes.BarkLoud or mode == MessageModes.NpcFromStartBlock) and
        creaturePos then
        ...
        if channelId == 9 then
            if channel then
                local coloredTextTable =
                    processLootMessage(composedMessage, speaktype.color)
                ...
                if #coloredTextTable > 0 then
                    ...
                    if tab then
                        local panel = consoleTabBar:getTabPanel(tab)
                        local consoleBuffer =
                            panel:getChildById('consoleBuffer')

                        local label = nil
                        if consoleBuffer:getChildCount() > MAX_LINES then
                            label = consoleBuffer:getFirstChild()
                            consoleBuffer:moveChildToIndex(label,
                                                           consoleBuffer:getChildCount())
                        end

                        if not label then
                            label = g_ui.createWidget('ConsoleLabel',
                                                      consoleBuffer)
                        end
                        ...
                        label.name = name
                        consoleBuffer.onMouseRelease =
                            function(self, mousePos, mouseButton)
                                processMessageMenu(mousePos, mouseButton, nil,
                                                   nil, nil, tab)
                            end
                        label.onMouseRelease =
                            function(self, mousePos, mouseButton)
                                processMessageMenu(mousePos, mouseButton, name,
                                                   composedMessage, self, tab)
                            end
                        label.onMousePress =
                        ...
                        label.onDragLeave =
                            function(self, droppedWidget, mousePos)
                                local text = {}
                                for selectionChild = consoleBuffer.selection
                                    .first, consoleBuffer.selection.last do
                                    local label =
                                        self:getParent():getChildByIndex(
                                            selectionChild)
                                    table.insert(text, label:getSelection())
                                end
                                consoleBuffer.selectionText = table.concat(text,
                            ...
                            function(self, mousePos, mouseMoved)
                                ...
                                if not child then
                                    if mousePos.y < self:getY() then
                                        for index = selfIndex - 1, 1, -1 do
                                            local label =
                                                parent:getChildByIndex(index)
                                            if label:getY() + label:getHeight() >
                                                parentRect.y then
                                                if (mousePos.y >= label:getY() and
                                                    mousePos.y <= label:getY() +
                                                    label:getHeight()) or index ==
                                                    1 then
                                                    child = label
                                                    break
                                                end
                                                ...
                                                child =
                                                    parent:getChildByIndex(
                                                        index + 1)
                                                break
                                            end
                                        end
                                    elseif mousePos.y > self:getY() +
                                        self:getHeight() then
                                        for index = selfIndex + 1, parent:getChildCount(), 1 do
                                            local label =
                                                parent:getChildByIndex(index)
                                            if label:getY() < parentRect.y +
                                                parentRect.height then
                                                if (mousePos.y >= label:getY() and
                                                    mousePos.y <= label:getY() +
                                                    label:getHeight()) or index ==
                                                    parent:getChildCount() then
                                ...
                                -- update self selection
                                local textBegin = self:getTextPos(
                                                      self:getLastClickPosition())
                                local textPos = self:getTextPos(mousePos)
                                self:setSelection(textBegin, textPos)
                                ...
                                if child ~= self then
                                    for selectionChild = consoleBuffer.selection
                                        .first + 1, consoleBuffer.selection.last -
                                        1 do
                                        parent:getChildByIndex(selectionChild)
                                            :selectAll()
                                    end
            ...
            -- Se o canal for o Advertising (id 5), tente usar o nome já registrado
            if channelId == 5 then
                local channelName = channels[5] or 'Advertising'
...
function onRuleViolationCancel(name)
    local tab = getTab(name .. '\'...')
    if not tab then return end
    addTabText(tr('%s has finished the request', name) .. '.',
               SpeakTypesSettings.privateRed, tab)
    tab.locked = true
end
...
function onRuleViolationLock()
    ...
    violationReportTab.locked = false
    addTabText(tr('Your request has been closed') .. '.',
               SpeakTypesSettings.privateRed, violationReportTab)
    violationReportTab.locked = true
end
...
function doChannelListSubmit()
    ...
    local openPrivateChannelWith = channelsWindow:getChildById(
                                       'openPrivateChannelWith'):getText()
...
function onChannelList(channelList)
    ...
    for k, v in pairs(channelList) do
        ...
        if #channelName > 0 then
            local label =
                g_ui.createWidget('ChannelListLabel', channelListPanel)
...
function loadCommunicationSettings()
    ...
    communicationSettings.privateMessages =
        g_settings.getBoolean('IgnorePrivateMessages')
...
function saveCommunicationSettings()
    ...
    g_settings.set('IgnorePrivateMessages',
                   communicationSettings.privateMessages)
...
function onClickIgnoreButton()
    ...
    local useIgnoreListBox = communicationWindow:getChildById(
                                 'checkboxUseIgnoreList')
    useIgnoreListBox:setChecked(communicationSettings.useIgnoreList)
    local useWhiteListBox = communicationWindow:getChildById(
                                'checkboxUseWhiteList')
    useWhiteListBox:setChecked(communicationSettings.useWhiteList)

    local removeIgnoreButton = communicationWindow:getChildById(
                                   'buttonIgnoreRemove')
    ...
    local removeWhitelistButton = communicationWindow:getChildById(
    ...
    local addWhitelistName = communicationWindow:getChildById(
                                 'whitelistNameEdit')
    local addWhitelistButton = communicationWindow:getChildById(
                                   'buttonWhitelistAdd')
    ...
    local ignorePrivateMessageBox = communicationWindow:getChildById(
                                        'checkboxIgnorePrivateMessages')
    ignorePrivateMessageBox:setChecked(communicationSettings.privateMessages)
    local ignoreYellingBox = communicationWindow:getChildById(
                                 'checkboxIgnoreYelling')
    ...
    saveButton.onClick = function()
        ...
        communicationSettings.privateMessages =
            ignorePrivateMessageBox:isChecked()
        communicationSettings.allowVIPs = allowVIPsBox:isChecked()
        communicationWindow:destroy()
    end

    local cancelButton = communicationWindow:recursiveGetChildById(
                             'buttonCancel')
...
function processLootMessage(message, defaultColor)
    ...
    local function containsAny(haystack, patterns)
        ...
        for _, p in ipairs(patterns) do if s:find(p) then return true end end
        return false
    end
...
function testLootChannelColor()
    ...
    if lootChannel then
        ...
        addTabText(
            "Loot de a dragon: rare sword, epic shield, legendary helmet, unidentified mace, 100 gold coins",
            SpeakTypesSettings.channelYellow, lootChannel, nil)
        addTabText(
            "Loot de a rotworm: *unidentified armor*, meat, 25 gold coins",
            SpeakTypesSettings.channelYellow, lootChannel, nil)
