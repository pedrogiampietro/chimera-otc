...
function onGameEditText(id, itemId, maxLength, text, writer, time)
  ...
  local textItem = textWindow:recursiveGetChildById('textItem')
  local textItemTitle = textWindow:recursiveGetChildById('textItemTitle')
  local description = textWindow:recursiveGetChildById('description')
  local textEdit = textWindow:recursiveGetChildById('text')
  local okButton = textWindow:recursiveGetChildById('okButton')
  local cancelButton = textWindow:recursiveGetChildById('cancelButton')

  local textScroll = textWindow:recursiveGetChildById('textScroll')

  if textItem:isHidden() then
    textItem:show()
  end

  if textItemTitle:isHidden() then
    textItemTitle:show()
  end

  textItem:setItemId(itemId)
  textItemTitle:setItemId(itemId)
...
function onGameEditList(id, doorId, text)
  ...
  local textItemTitle = textWindow:recursiveGetChildById('textItemTitle')
  if textItemTitle and not textItemTitle:isHidden() then
    textItemTitle:hide()
  end
