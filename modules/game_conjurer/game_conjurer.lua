...
local currentConjurerData = {experience = 0, nextLevelExp = 500, level = 0}
...
if not json then
    ...
    function json.decode(str)
        g_logger.warning(
            "Implementação json.decode simplificada - retornando string original")
...
if not g_logger then
    g_logger = {}
    function g_logger.info(message) print("[INFO] " .. message) end
    function g_logger.warning(message) print("[WARNING] " .. message) end
    function g_logger.error(message) print("[ERROR] " .. message) end
end

function getCurrentConjurerLevel() return currentConjurerLevel end

function init()
    conjurerWindow = g_ui.displayUI('game_conjurer',
    ...
    for i = 0, 5 do
        local rankIconWidget = conjurerWindow:recursiveGetChildById(
                                   rankIconIds[i])
    ...
    if type(registerExtendedOpcode) == 'function' then
        ...
        if ProtocolGame and ProtocolGame.registerExtendedOpcode then
            ...
            ProtocolGame.registerExtendedOpcode(ExtendedIds.ConjurerRanking,
                                                onConjurerRanking)
            ProtocolGame.registerExtendedOpcode(ExtendedIds.ConjurerData,
                                                onConjurerData)
            ProtocolGame.registerExtendedOpcode(ExtendedIds.ConjurerGemData,
                                                onConjurerGemData)
            ProtocolGame.registerExtendedOpcode(ExtendedIds.ConjurerGemSync,
                                                onConjurerGemSync)
        else
...
function displayRankRequirements(rankIndex)
    ...
    modules.game_textmessage.displayGameMessage(tr('Locked: ') ..
...
function toggleConjurerRankingWindow()
    ...
    if not conjurerRankingWindow then
        ...
        local closeButton = conjurerRankingWindow:recursiveGetChildById(
                                'buttonClose') or
                                conjurerRankingWindow:recursiveGetChildById(
                                    'closeButton')
        if closeButton then
        ...
        local backButton = conjurerRankingWindow:recursiveGetChildById(
                               'buttonBack') or
                               conjurerRankingWindow:recursiveGetChildById(
                                   'backButton')
        if backButton then
    ...
    if conjurerRankingWindow:isVisible() then
        ...
        if conjurerWindow then conjurerWindow:hide() end
...
function refreshConjurerRanking()
    ...
    if not conjurerRankingWindow then
        g_logger.warning(
            "Tentativa de atualizar ranking, mas a janela não foi inicializada.")
        return
    end
    ...
    elseif g_game and g_game.sendExtendedOpcode then
        ...
        g_logger.warning(
            "Função sendExtendedOpcode não disponível para atualizar ranking.")
...
function setupTopMenuButton()
    if not conjurerButton then
        conjurerButton = modules.client_topmenu.addRightGameToggleButton(
                             'conjurerButton', tr('Conjurer'),
                             '/images/topbuttons/new/conjurer',
                             toggleConjurerWindow, nill, false, 4)
        conjurerButton:setOn(false)
    end
end
...
function updateConjurerUI(level, exp, nextExp)
    ...
    if currentLevelLabel then
        ...
        currentLevelLabel:setText(tr('Current Level: ') .. level .. ' - ' ..
                                      tr(rankName))
    end
    ...
    for i = 0, 5 do
        ...
        local rankBonusWidget = conjurerWindow:recursiveGetChildById(
                                    rankIds[i] .. 'Bonus')
        local rankDateWidget = conjurerWindow:recursiveGetChildById(
                                   rankIds[i] .. 'Date')
            ...
            if i <= level then
                ...
                if i == level then
                    ...
                    scheduleEvent(function()
                        pulsateRankIcon(rankIconWidget)
                    end, 500)
                else
    ...
    local percent = 0
    if nextExp > 0 then percent = math.min(1, exp / nextExp) end
    ...
    scheduleEvent(function()
        local rankDetailsContent = conjurerWindow:recursiveGetChildById(
                                       'rankDetailsContent')
...
local function safeTonumber(val)
    if type(val) == 'string' then
        ...
        if cleaned then return tonumber(cleaned) end
...
function showRankDetails(rankId)
    ...
    if rankId < 5 then
        local progressBarBg = conjurerWindow:getChildById('progressBarBg')
        local progressLabel = progressBarBg and
                                  progressBarBg:getChildById('progressLabel')
    ...
    local existingWindow = g_ui.getRootWidget():recursiveGetChildById(
                               'rankDetailsWindow')
    if existingWindow then existingWindow:destroy() end

    -- Carregar o modal a partir do novo arquivo OTUI
    local windowRoot = g_ui.displayUI('game_conjurer_rankdetails')
    local window = windowRoot:recursiveGetChildById('rankDetailsWindow') or
                       windowRoot
    ...
    if backButton then backButton.onClick = function() window:destroy() end end

    local closeButton = window:recursiveGetChildById('rankDetailsCloseButton')
    if closeButton then closeButton.onClick = function() window:destroy() end end
end
...
function animateRankUnlock(widget)
    ...
    local function pulse(step)
        ...
        scheduleEvent(function()
            ...
            scheduleEvent(function() pulse(step + 1) end, 80)
        end, 80)
    end
    pulse(1)
end
...
function setupRankIconClicks()
    ...
    for i = 0, 5 do
        ...
        if icon then icon.onClick = function() showRankDetails(i) end end
    end
end
...
function setupConjurerWindow()
    ...
    rankButton:setAnchors({
        bottom = 'buttonClose.bottom',
...
function setupRankGemSystem(content, rankId)
    ...
    if slotContainer then
        slotContainer.onDrop = function(self, dragged)
            g_logger.info(string.format(
                              "[GEM-DROP-CONTAINER] onDrop chamado no gem_slot_container do rank %s. Dragged: %s",
                              tostring(rankId), dragged and dragged.gemData and
                                  dragged.gemData.name or "N/A"))
            return false
        end
    end
    ...
    elseif rankId == currentLevel then
        ...
        local expPercentage = nextLevelExp > 0 and (currentExp / nextLevelExp) *
                                  100 or 0
    ...
    for i = 1, 3 do
        ...
        local gemSlotLabel = content:recursiveGetChildById('gemSlot' .. i ..
                                                               '_label')
        ...
        if gemSlotBox then
            gemSlotBox.onDrop = function(self, dragged)
                g_logger.info(string.format(
                                  "[GEM-DROP-BOX] onDrop chamado no gem_slot_box%s do rank %s. Dragged: %s",
                                  tostring(i), tostring(rankId),
                                  dragged and dragged.gemData and
                                      dragged.gemData.name or "N/A"))
                return false
            end
        end
        ...
        if gemSlot and gemSlotBox then
            if i <= slotsUnlocked then
                ...
                gemSlot.onDrop = function(self, dragged)
                    g_logger.info(string.format(
                                      "[GEM-DROP] onDrop chamado no slot %s do rank %s. Dragged: %s",
                                      tostring(i), tostring(rankId), dragged and
                                          dragged.gemData and
                                          dragged.gemData.name or "N/A"))
                    -- Verificar se é uma gema válida
                    if not dragged or not dragged.gemData then
                        g_logger.warning(
                            "[GEM-DROP] Dragged ou gemData inválido no drop!")
                        return false
                    end
                ...
                if rankId > currentLevel then
                    ...
                    tooltipText = 'Unlocked with ' .. tostring(expPercent) ..
                                      '% EXP in rank'
                end
...
function requestGemsFromServer(rankId)
    ...
    if protocolGame then
        ...
        protocolGame:sendExtendedOpcode(ExtendedIds.ConjurerGemRequest,
                                        json.encode(data))
    end
end
...
function updateGemsPanel(gemsData, currentRankId)
    ...
    local content = g_ui.getRootWidget():recursiveGetChildById(
                        'rankDetailsContent')
    if not content then return end

    local gemsPanel = content:recursiveGetChildById('availableGemsPanel')
    if not gemsPanel then return end
    ...
    for _, gemData in ipairs(gemsData) do
        ...
        if not isEquippedElsewhere then
            ...
            if gemInfo then
                ...
                imageArea:addAnchor(AnchorHorizontalCenter, 'parent',
                                    AnchorHorizontalCenter)
                imageArea:addAnchor(AnchorVerticalCenter, 'parent',
                ...
                if gemWidget then
                    ...
                    gemWidget:addAnchor(AnchorHorizontalCenter, 'parent',
                                        AnchorHorizontalCenter)
                    gemWidget:addAnchor(AnchorVerticalCenter, 'parent',
                                        AnchorVerticalCenter)

                    -- Carregar imagem da gema
                    local imagePath = string.format('/images/items/%d.png',
                                                    itemId)
                    ...
                    local tooltipText = string.format(
                                            ...
                                            gemInfo.name or '-',
                                            (gemInfo.bonus and
                                                gemInfo.bonus:upper() or '-'),
                                            tostring(gemInfo.value or '-'),
                                            (count and count > 1) and
                                                (string.format('\nAmount: %d',
                                                               count)) or '')
                    ...
                    if count > 1 then
                        local countContainer =
                            g_ui.createWidget('UIWidget', gemContainer)
                        countContainer:setSize({width = 18, height = 16})
                        countContainer:addAnchor(AnchorRight, 'parent',
                                                 AnchorRight)
                        ...
                        -- Gradiente interno
                        local countInner =
                            g_ui.createWidget('UIWidget', countContainer)
                        countInner:setSize({width = 16, height = 14})
                        countInner:addAnchor(AnchorHorizontalCenter, 'parent',
                                             AnchorHorizontalCenter)
                        countInner:addAnchor(AnchorVerticalCenter, 'parent',
                                             AnchorVerticalCenter)
                    ...
                    gemWidget.onDoubleClick = function(self)
                        g_logger.info(string.format(
                                          "[GEM-EQUIP-CLICK] Clique duplo em gema: %s (itemId: %s)",
                                          self and self.gemData and
                                              self.gemData.name or "N/A",
                                          self and self.gemData and
                                              tostring(self.gemData.itemId) or
                                              "N/A"))
                        -- Procurar o primeiro slot livre
                        local content =
                            g_ui.getRootWidget():recursiveGetChildById(
                                'rankDetailsContent')
                        if not content then return end
                        local rankId = content.rankId or currentRankIdForGems or
                                           0
                        for slotNum = 1, 3 do
                            local slot =
                                content:recursiveGetChildById('gemSlot' ..
                                                                  slotNum)
                            if slot and not slot.equippedGem then
                                g_logger.info(string.format(
                                                  "[GEM-EQUIP-CLICK] Equipando gema '%s' no slot %d do rank %d",
                                                  self.gemData.name, slotNum,
                                                  rankId))
...
function handleGemDrop(slot, dragged, rankId, slotNumber)
    ...
    if not dragged.gemData then
        ...
        if not dragged.gemData.count then dragged.gemData.count = 1 end
    end
    ...
    if slot:getOpacity() < 0.5 or slot:getImageColor() == '#555555' then
        modules.game_textmessage.displayGameMessage(
            'This slot is locked. Unlock it to equip a gem.', 'Conjurer System')
        return false
    end
    if slot.equippedGem then
        modules.game_textmessage.displayGameMessage(
            'Remove the current gem first!', 'Conjurer System')
        return false
    end
    ...
    if not localEquippedGems[rankId] then localEquippedGems[rankId] = {} end
    ...
    g_logger.info(string.format(
                      "[GEM-EQUIP] Gema salva localmente: %s (slot %s, rank %s)",
                      gem.name, tostring(slotNumber), tostring(rankId)))
    ...
    local content = g_ui.getRootWidget():recursiveGetChildById(
                        'rankDetailsContent')
    if content then setupAvailableGems(content, rankId) end
...
function removeGemFromSlot(rankId, slotNumber)
    ...
    if not slot.equippedGem then return false end
    ...
    modules.game_textmessage.displayGameMessage(
        tr('Gema removida com sucesso!'), 'Conjurer System')
...
function updateRankGemsEffects()
    ...
    for i = 1, 3 do
        ...
        if slot and slot.equippedGem then
            ...
            if not bonusTotals[bonus] then bonusTotals[bonus] = 0 end
    ...
    if #equippedGems == 0 then
        ...
        for bonusType, totalValue in pairs(bonusTotals) do
            ...
            table.insert(effects,
                         icon .. "+" .. totalValue .. " " .. bonusType:upper())
        end
    ...
    if #equippedGems > 0 then
        ...
        for i, gem in ipairs(equippedGems) do
            tooltipText = tooltipText .. "Slot " .. i .. ": " .. gem.name ..
                              "\n"
            tooltipText = tooltipText .. "  > +" .. gem.value .. " " ..
                              gem.bonus:upper() .. "\n\n"
        end

        tooltipText = tooltipText ..
                          "💡 Clique no botão '?' para análise detalhada"
...
function loadRankGems(rankId)
    ...
    if g_game.isOnline() and g_game.sendExtendedOpcode then
        ...
        g_game.sendExtendedOpcode(ExtendedIds.ConjurerGemRequest,
                                  json.encode(data))
    end
end
...
function analyzeDamageForGemBonus(damageValue)
    ...
    if #recentDamages < 3 then
        ...
        g_logger.info(string.format(
                          "🔍 SIMPLE ANALYSIS - Damage: %d, Threshold: %.1f, Bonus: +%d, Activated: %s",
                          damageValue, threshold, currentAttackBonus,
                          tostring(activated)))
        return activated
    end
    ...
    local sum = 0
    for _, dmg in ipairs(recentDamages) do sum = sum + dmg end
    ...
    local percentil50Value = recentDamages[percentil50Index] or
                                 gemBonusTracker.baseDamageRange.max
    ...
    if not isBonusActivated and currentAttackBonus > 0 then
        ...
        if randomChance <= bonusChance then
            ...
            g_logger.info(string.format(
                              "💎 BONUS ACTIVATED BY GEM CHANCE (%d%% with +%d ATK)",
                              bonusChance, currentAttackBonus))
        end
    end

    g_logger.info(string.format(
                      "💥 DAMAGE ANALYSIS - Value: %d, Avg: %.1f, Threshold: %.1f, P50: %d, Bonus: +%d, Activated: %s",
                      damageValue, average, threshold, percentil50Value,
                      currentAttackBonus, tostring(isBonusActivated)))
    ...
    if localEquippedGems[currentRank] then
        for slotNumber, gemData in pairs(localEquippedGems[currentRank]) do
            if gemData and gemData.bonus == "atk" then
                ...
                g_logger.info(string.format(
                                  "🔍 Found ATK gem in slot %d: +%d (total: %d)",
...
function createBonusDamageMessage(targetName, bonusDamage, bonusValue)
    if not gemBonusTracker.clientVisualsEnabled then return end
    -- Criar mensagem de dano bônus SEPARADO como segundo hit (igual na imagem)
    local bonusMessage = string.format(
                             "💎 %s loses %d hitpoints due to gem bonus! (ATK +%d)",
    ...
    if g_game and g_game.processTextMessage then
        ...
        scheduleEvent(function()
            local secondHitMsg = string.format(
                                     "A %s loses %d hitpoints due to gem magic.",
                                     targetName, bonusDamage)
            g_game.processTextMessage(16, secondHitMsg) -- Same mode as normal damage but delayed
        end, 200)
    end

    -- Log para debug
    g_logger.info("💎 DUAL HIT - First: Original damage, Second: +" ..
                      bonusDamage .. " gem bonus")
end
...
function showBonusDamageText(bonusDamage, bonusValue)
    ...
    local bonusMessage =
        string.format("💎 +%d gem bonus damage!", bonusDamage)
    ...
    g_logger.info("💎💎 Bonus damage displayed: +" .. bonusDamage ..
                      " (ATK +" .. bonusValue .. ")")
end
...
function showGemBonusEffect(damageValue, bonusValue)
    ...
    scheduleEvent(function() showBonusDamageText(bonusDamage, bonusValue) end,
                  300) -- Delay para aparecer após o dano principal
    ...
    g_logger.info("🔥 GEM BONUS ACTIVATED - Base: " .. damageValue ..
                      ", Bonus: +" .. bonusDamage)
    ...
    animateIn = function()
        ...
        local bounce = progress < 0.7 and progress or 0.7 +
                           (math.sin((progress - 0.7) * 10) * 0.1)
    ...
    modules.game_textmessage.displayGameMessage(
        '� CRÍTICO! Gema de Ataque ativou bônus de +' .. bonusValue ..
            ' ATK!', 'Gem Power')

    -- Log para debug
    g_logger.info("🔥 GEM BONUS ACTIVATED - Damage: " .. damageValue ..
                      ", Bonus: +" .. bonusValue)
end
...
function createSingleParticle(parentWidget, color, icon, index)
    ...
    animateParticle = function()
        ...
        local newX = parentPos.x + 125 +
                         (math.cos(angle + progress * 2) * newRadius)
        local newY = parentPos.y + 40 +
                         (math.sin(angle + progress * 2) * newRadius)
...
function animateGemEffectExit(widget)
    ...
    animateOut = function()
        ...
        widget:setSize({width = 250 * progress, height = 80 * progress})
...
function testGemEffect(gemType, damageValue, bonusValue)
    ...
    g_logger.info("Testing gem effect - Type: " .. gemType .. ", Damage: " ..
                      damageValue .. ", Bonus: " .. bonusValue)
    showGemBonusEffect(damageValue, bonusValue)

    modules.game_textmessage.displayGameMessage(
        '🧪 Teste de efeito de gema executado!', 'Gem Debug')
end

-- Função para simular dano para teste
function simulateDamageForTest(damageValue)
    damageValue = damageValue or 65
    processMessageForGemBonus(16, "A dragon loses " .. damageValue ..
                                  " hitpoints due to your attack.")
end
...
function loadLocalEquippedGems(content, rankId)
    ...
    for slotNumber, gemData in pairs(localEquippedGems[rankId]) do
        ...
        if slot and gemData then
            g_logger.info("Restoring gem in slot " .. slotNumber .. ": " ..
                              gemData.name)
    ...
    scheduleEvent(function() updateRankGemsEffects() end, 100)
end

-- Função para alternar modo de teste
function toggleTestMode(enabled)
    if enabled == nil then enabled = not gemBonusTracker.testMode end
    ...
    modules.game_textmessage.displayGameMessage(
        "Modo teste: " .. status .. " (chance aumentada de ativação)",
        'Gem System')
...
function setupGemDamageMonitoring()
    ...
    if modules.game_textmessage and modules.game_textmessage.onGameMessage then
        ...
        modules.game_textmessage.onGameMessage =
            function(mode, text)
        ...
        g_logger.info(
            "Gem damage monitoring system activated via game_textmessage")
    else
        g_logger.warning(
            "Could not hook game_textmessage, trying alternative methods")
        ...
        if g_logger and g_logger.info then
            ...
            g_logger.info = function(message)
                ...
                if message and message:find("DEBUG displayMessage") and
                    message:find("Mode: 16") then
                    local text = message:match("Text: (.+)")
                    if text then
                        g_logger.info(
                            "🔄 Intercepted DEBUG message, processing: " ..
                                text)
                ...
                if message and message:find("🎯 DAMAGE DETECTED:") then
                    local damageMatch = message:match(
                                            "🎯 DAMAGE DETECTED: (%d+)")
                    if damageMatch then
                        local damage = tonumber(damageMatch)
                        g_logger.info("🔄 Re-processing detected damage: " ..
                                          damage)

                        local attackBonus = getCurrentAttackBonus()
                        if attackBonus > 0 then
                            local isBonusActivated =
                                analyzeDamageForGemBonus(damage)
                            if isBonusActivated then
                                local bonusDamage = math.floor(damage *
                                                                   (attackBonus /
                                                                       100))
                                scheduleEvent(function()
                                    createBonusDamageMessage("target",
                                                             bonusDamage,
                                                             attackBonus)
                                end, 200)
            ...
            g_logger.warning(
                "No suitable method found for gem damage monitoring")
...
function processMessageForGemBonus(mode, text)
    ...
    for _, pattern in ipairs(damagePatterns) do
        ...
        if match then
            ...
            targetName = text:match("A?n? ?(.+) loses %d+ hitpoints") or
                             "target"
            break
        end
    end
    ...
    if damageValue then
        g_logger.info(
            "🎯 DAMAGE DETECTED: " .. damageValue .. " from text: " .. text)

        -- Verificar se o sistema de gemas está ativo
        local attackBonus = getCurrentAttackBonus()
        g_logger.info("🔧 DEBUG - Attack bonus: " .. attackBonus ..
                          ", Gem tracker enabled: " ..
                          tostring(gemBonusTracker.enabled))
        ...
        if attackBonus > 0 then
            ...
            g_logger.info("🔍 Bonus activation result: " ..
                              tostring(isBonusActivated))
            ...
            if isBonusActivated then
                ...
                g_logger.info("❌ Gem bonus not activated for damage: " ..
                                  damageValue)
            end
        else
...
function simulateDamageFromLogs()
    local damages = {
        67, 67, 14, 69, 60, 71, 65, 57, 56, 68, 57, 72, 61, 62, 59, 54, 63
    }
    ...
    for i, dmg in ipairs(damages) do
        scheduleEvent(function()
            g_logger.info(
                string.format("🎯 Processing damage #%d: %d", i, dmg))
...
function testGemBonusEffect()
    ...
    if attackBonus > 0 then
        g_logger.info(
            "💎 Testing gem bonus effect with current ATK bonus: +" ..
                attackBonus)
        showGemBonusEffect(65, attackBonus)

        -- Testar também a mensagem de dano bônus
        scheduleEvent(function()
            createBonusDamageMessage("hydra",
                                     math.floor(65 * (attackBonus / 100)),
                                     attackBonus)
        end, 500)
    else
        g_logger.warning(
            "No attack bonus found! Make sure you have a gem equipped.")
    end
end
...
function testMessageColors()
    ...
    local colors = {
        "Normal", "Orange", "Red/Critical", "White", "Green", "Blue", "Yellow"
    }

    for i, mode in ipairs(modes) do
        scheduleEvent(function()
            local testMessage = string.format(
                                    "Mode %d: %s - Test damage message", mode,
                                    colors[i] or "Unknown")
            if g_game and g_game.processTextMessage then
...
function debugConnectionStatus()
    ...
    g_logger.info("  - g_game.sendExtendedOpcode available: " ..
                      tostring(g_game.sendExtendedOpcode ~= nil))

    if g_game.getLocalPlayer then
        local localPlayer = g_game.getLocalPlayer()
        g_logger.info("  - Local player exists: " ..
                          tostring(localPlayer ~= nil))
        if localPlayer then
            g_logger.info("  - Player name: " ..
                              (localPlayer:getName() or "no name"))
        end
    end

    if g_game.getProtocolVersion then
        g_logger.info("  - Protocol version: " ..
                          tostring(g_game.getProtocolVersion()))
    end
end
...
function checkServerGemStatus()
    ...
    if g_game.isOnline() and g_game.sendExtendedOpcode then
        ...
        g_logger.info(
            "🔧 Testing mode: server status check not available (using local data)")
        -- Mostrar status local das gemas
        local totalGems = 0
        for rank, gems in pairs(localEquippedGems) do
            for slot, gem in pairs(gems) do totalGems = totalGems + 1 end
        end
...
function testCompleteGemSystem()
    ...
    scheduleEvent(function() testGemSync() end, 1000)

    -- 3. Verificar status do servidor
    scheduleEvent(function() checkServerGemStatus() end, 2000)

    -- 4. Testar efeito visual
    scheduleEvent(function() testGemBonusEffect() end, 3000)
...
function forceServerCommunicationTest()
    ...
    if protocolGame then
        ...
        local success, error = pcall(function()
            protocolGame:sendExtendedOpcode(ExtendedIds.ConjurerGemSync,
                                            jsonData)
        end)

        if success then
            g_logger.info(
                "✅ Test ping sent successfully via protocolGame - check server console!")
        else
            g_logger.error("❌ Failed to send ping via protocolGame: " ..
                               tostring(error))
        end
    elseif g_game and g_game.sendExtendedOpcode then
        ...
        if success then
            g_logger.info(
                "✅ Test ping sent successfully via g_game - check server console!")
        else
            g_logger.error("❌ Failed to send ping via g_game: " ..
                               tostring(error))
        end
    else
    ...
    for rank, gems in pairs(localEquippedGems) do
        for slot, gem in pairs(gems) do gemsEquipped = gemsEquipped + 1 end
    end

    if gemsEquipped > 0 then
        g_logger.info("💎 Found " .. gemsEquipped ..
                          " equipped gems - attempting force sync...")
...
function debugGemSystem()
    ...
    for rank, gems in pairs(localEquippedGems) do
        ...
        for slot, gemData in pairs(gems) do
            g_logger.info(string.format(
                              "    Slot %d: %s (bonus: %s, value: %d)", slot,
                              gemData.name or "Unknown",
                              gemData.bonus or "none", gemData.value or 0))
        end
    end
...
function syncGemsWithServer(rankId)
    ...
    local equippedGemsData = {rank = rankId, gems = {}}
    ...
    for slotNumber, gemData in pairs(localEquippedGems[rankId]) do
        ...
        if slot then
            ...
            g_logger.info("📦 Prepared gem for sync: slot=" .. slot ..
                              ", itemId=" .. gemData.itemId .. ", bonus=" ..
                              gemData.bonus)
        else
            g_logger.warning("⚠️ Invalid slot number: " ..
                                 tostring(slotNumber))
        end
    end

    -- Tentar enviar para servidor via opcode ConjurerGemSync (usando mesmo método do ranking)
    local data = {action = "syncEquippedGems", data = equippedGemsData}
    ...
    if protocolGame then
        ...
        if success then
            g_logger.info(
                "✅ Gems sync sent successfully via protocolGame for rank " ..
                    rankId .. " (" .. #equippedGemsData.gems .. " gems)")
        else
            g_logger.error("❌ Failed to send via protocolGame: " ..
                               tostring(error))
        end
    elseif g_game and g_game.sendExtendedOpcode then
        ...
        if success then
            g_logger.info(
                "✅ Gems sync sent successfully via g_game for rank " .. rankId ..
                    " (" .. #equippedGemsData.gems .. " gems)")
        else
            g_logger.error("❌ Failed to send via g_game: " .. tostring(error))
        end
    else
        g_logger.error(
            "❌ No method available to send gem sync - both protocolGame and g_game.sendExtendedOpcode unavailable")
    end
end
...
function removeGemFromServer(rankId, slotNumber)
    local data = {
        action = "removeGem",
        data = {rank = rankId, slot = slotNumber}
    }
    ...
    if protocolGame then
        ...
        if success then
            g_logger.info(
                "✅ Gem removal sent successfully - Rank: " .. rankId ..
                    ", Slot: " .. slotNumber)
        else
            g_logger.error(
                "❌ Failed to send gem removal via protocolGame: " ..
                    tostring(error))
        end
    elseif g_game and g_game.sendExtendedOpcode then
        ...
        if success then
            ...
            g_logger.error("❌ Failed to send gem removal via g_game: " ..
                               tostring(error))
        end
    else
...
function toggleGemBonusMonitoring(enabled)
    if enabled == nil then enabled = not gemBonusTracker.enabled end
    ...
    modules.game_textmessage.displayGameMessage(
        "Monitoramento de gemas: " .. status, 'Gem System')
...
function calibrateBaseDamage(minDamage, maxDamage)
    ...
    g_logger.info("Base damage range calibrated: " .. minDamage .. "-" ..
                      maxDamage)
    modules.game_textmessage.displayGameMessage(
        "Dano base calibrado: " .. minDamage .. "-" .. maxDamage, 'Gem System')
end
...
function showDamageHistory()
    if #gemBonusTracker.damageHistory == 0 then
        modules.game_textmessage.displayGameMessage(
            "Nenhum histórico de dano disponível", 'Gem System')
        return
    end
    ...
    local historyText = "📊 HISTÓRICO DE DANOS (Últimos " ..
                            #gemBonusTracker.damageHistory .. " ataques)\n\n"
    ...
    local avgBonusDamage = withBonusCount > 0 and
                               (withBonusDamage / withBonusCount) or 0

    historyText = historyText .. string.format("📈 ESTATÍSTICAS:\n")
    historyText = historyText ..
                      string.format("• Dano médio geral: %.1f\n", avgDamage)
    historyText = historyText ..
                      string.format("• Dano médio com bônus: %.1f\n",
                                    avgBonusDamage)
    historyText = historyText ..
                      string.format(
                          "• Ataques com bônus: %d/%d (%.1f%%)\n\n",
                          withBonusCount, #gemBonusTracker.damageHistory,
                          (withBonusCount / #gemBonusTracker.damageHistory) *
                              100)
    ...
    for i = startIndex, #gemBonusTracker.damageHistory do
        ...
        historyText = historyText ..
                          string.format("%d. [%s] %d dmg%s\n", i, timeStr,
                                        record.damage, bonusStr)
    end

    -- Análise de padrões
    historyText = historyText .. "\n🔍 ANÁLISE:\n"
    historyText = historyText ..
                      string.format("• Faixa base configurada: %d-%d\n",
                                    gemBonusTracker.baseDamageRange.min,
                                    gemBonusTracker.baseDamageRange.max)

    local currentBonus = getCurrentAttackBonus()
    if currentBonus > 0 then
        historyText = historyText ..
                          string.format("• Bônus atual: +%d ATK\n",
                                        currentBonus)
        historyText = historyText .. string.format("• Dano esperado: %d-%d\n",
                                                   gemBonusTracker.baseDamageRange
                                                       .min + currentBonus,
                                                   gemBonusTracker.baseDamageRange
                                                       .max + currentBonus)
    else
    ...
    closeButton.onClick = function() historyWindow:destroy() end
...
function createGemEffectsMonitor()
    ...
    local existingMonitor = g_ui.getRootWidget():recursiveGetChildById(
                                'gemEffectsMonitor')
...
function onConjurerGemData(protocol, opcode, buffer)
    g_logger.info("Received ConjurerGemData opcode with buffer length: " ..
                      buffer:len())
    ...
    if not success or not data then
        g_logger.error("Failed to decode ConjurerGemData JSON: " ..
                           tostring(data))
        return
    end
    ...
    if data.action == "gemList" and data.gems then
        g_logger.info("Received gems list from server with " .. #data.gems ..
                          " gem types")
    ...
    if data.action == "statsResponse" and data.stats then
        ...
        g_logger.info("Server Health Regen: " ..
                          (data.stats.healthregen or "N/A"))
        ...
        if totalBonuses then
            ...
            for bonusType, expectedValue in pairs(totalBonuses) do
                ...
                g_logger.info(bonusType:upper() .. " - Expected: +" ..
                                  expectedValue .. ", Server: " .. serverValue)
            end
        end

        modules.game_textmessage.displayGameMessage(
            "Stats do servidor recebidos! Veja console para detalhes.",
            'Gem Debug')
        return
    end
    ...
    local content = g_ui.getRootWidget():recursiveGetChildById(
                        'rankDetailsContent')
    if content then updateGemSlotsDisplay(content, data) end
end
    ...
    for i = 1, 3 do
        ...
        if slot then
            ...
            if equippedItemId and equippedItemId > 0 then
                ...
                slot.equippedGem = nil
                slot:setTooltip(tr(
                                    'Arraste uma gema aqui ou clique direito para remover'))
            end
        end
    end
...
function onInventoryChange(slot, item, oldItem)
    ...
    if item and type(item) == 'userdata' and item.getId then
        ...
        if itemId == 5273 or itemId == 5274 or itemId == 5275 or itemId == 5276 or
            itemId == 5277 then isGemItem = true end
    end
    ...
    if oldItem and type(oldItem) == 'userdata' and oldItem.getId then
        ...
        if oldItemId >= 2147 and oldItemId <= 2152 then isGemItem = true end
    end
    ...
    if isGemItem then
        ...
        local content = g_ui.getRootWidget():recursiveGetChildById(
                            'rankDetailsContent')
        if content then setupAvailableGems(content) end
    end
end
...
function debugGemBonuses()
    ...
    if #activeGems > 0 then
        ...
        for bonusType, totalValue in pairs(totalBonuses) do
            message = message .. "▶ " .. bonusType:upper() .. ": +" ..
                          totalValue .. "\n"
        end
        message = message .. "\n💎 Gemas equipadas: " .. #activeGems
        modules.game_textmessage.displayGameMessage(message, 'Gem Debug')
    else
        modules.game_textmessage.displayGameMessage("Nenhuma gema equipada",
                                                    'Gem Debug')
    end
...
function compareStatsWithGems()
    ...
    g_logger.info("Current Mana: " .. currentStats.mana .. "/" ..
                      currentStats.maxMana)
    g_logger.info("Current Health: " .. currentStats.health .. "/" ..
...
function showGemEffectsDetailed()
    ...
    if not activeGems or #activeGems == 0 then
        modules.game_textmessage.displayGameMessage(
            "Nenhuma gema equipada para analisar", 'Gem Debug')
        return
    end
    ...
    for _, gem in ipairs(activeGems) do
        detailText = detailText ..
                         string.format("Slot %d: %s (ID: %d)\n", gem.slot,
                                       gem.name, gem.itemId)
        detailText = detailText ..
                         string.format("  ▶ Bônus: +%d %s\n\n", gem.value,
                                       gem.bonus:upper())
    end
    ...
    captureButton.onClick = function()
        ...
        modules.game_textmessage.displayGameMessage(
            "Stats capturados! Veja o console para detalhes.", 'Gem Debug')
    end
    ...
    testButton.onClick = function() requestStatsComparison() end
    ...
    consoleButton.onClick = function() debugGemBonuses() end
    ...
    monitorButton:setText(gemBonusTracker.enabled and 'Disable Monitor' or
                              'Enable Monitor')
    ...
    calibrateButton.onClick = function()
        ...
        if #gemBonusTracker.damageHistory >= 3 then
            ...
            if #damages >= 2 then
                ...
                modules.game_textmessage.displayGameMessage(
                    "Precisa de mais dados sem gemas equipadas", 'Gem System')
            end
        else
            modules.game_textmessage.displayGameMessage(
                "Histórico insuficiente para calibração", 'Gem System')
        end
    end
    ...
    historyButton.onClick = function() showDamageHistory() end

    -- Botão modo teste - Terceira linha
    local testModeButton = g_ui.createWidget('Button', window)
    testModeButton:setText(gemBonusTracker.testMode and 'Test Mode: ON' or
                               'Test Mode: OFF')
    ...
    connectionTestButton.onClick = function() forceServerCommunicationTest() end
...
function onConjurerGemSync(protocol, opcode, buffer)
    g_logger.info("📨 Received ConjurerGemSync opcode with buffer length: " ..
                      buffer:len())
    ...
    if not success or not data then
        g_logger.error("Failed to decode ConjurerGemSync JSON: " ..
                           tostring(data))
        return
    end
    ...
    if data.status == "success" then
        if data.message then g_logger.info("✅ " .. data.message) end
        if data.totalAttackBonus then
            g_logger.info("⚔️ Server confirmed ATK bonus: +" ..
                              data.totalAttackBonus)
        end
        g_logger.info("✅ Gems synchronized successfully with server!")
    elseif data.status == "error" then
        g_logger.warning("❌ Server gem sync error: " ..
...
function onConjurerData(protocol, opcode, buffer)
    ...
    function createDualHitEffect(baseDamage, bonusDamage, targetName)
        if not gemBonusTracker.clientVisualsEnabled or
            not dualHitTracker.enabled then return end
        g_logger.info("💥 DUAL HIT EFFECT - Base: " .. baseDamage ..
                          ", Bonus: " .. bonusDamage)

        -- First hit: Show original damage (already shown by game)
        -- Second hit: Show bonus damage with slight delay
        scheduleEvent(function()
            local bonusHitMsg = string.format(
                                    "A %s loses %d hitpoints due to magical enhancement.",
                                    targetName, bonusDamage)
            ...
            local notificationMsg = string.format(
                                        "💎 Gem Bonus Hit: +%d damage!",
                                        bonusDamage)
            if modules.game_textmessage and
                modules.game_textmessage.displayMessage then
    ...
    function processDualHitDamage(damageValue, targetName)
        if not gemBonusTracker.clientVisualsEnabled or
            not dualHitTracker.enabled then return false end
        local attackBonus = getCurrentAttackBonus()
        ...
        g_logger.info(
            "⚔️ DUAL HIT PROCESSING - Original: " .. damageValue ..
                ", Bonus: +" .. bonusDamage)
    ...
    function testDualHitSystem()
        ...
        for i, damage in ipairs(testDamages) do
            scheduleEvent(function()
                g_logger.info("Testing dual hit #" .. i .. " with damage: " ..
                                  damage)
