...
local conjurerRanks = {
    'Novice', 'Apprentice', 'Adept', 'Master', 'Elder', 'Legendary',
...
local function checkExtendedOpcodesAvailability()
    local result = {
        ...
        sendAvailable = (g_game and type(g_game.sendExtendedOpcode) ==
            'function'),
        protocolEnabled = nil
    }
    ...
    if result.sendAvailable then
        ...
        if not success then
            g_logger.warning("Extended opcodes protocol test failed: " ..
                                 tostring(error))
        end
    end

    g_logger.info("Extended opcodes availability check: " .. "Register: " ..
                      tostring(result.registerAvailable) .. ", Send: " ..
                      tostring(result.sendAvailable) .. ", Protocol: " ..
                      tostring(result.protocolEnabled))

    return result
end

function init()
    connect(g_game, {onGameEnd = hide, onGameStart = requestConjurerData})
    ...
    local function safeRegister(opcode, handler)
        if extendedOpcodesStatus.registerAvailable then
            ...
            if not success then
                g_logger.error("Failed to register opcode " .. opcode .. ": " ..
                                   tostring(error))
            else
    ...
    local mainWindow = modules.game_interface.getRightPanel():getChildById(
                           'conjurerWindow')
    if mainWindow then
...
function terminate()
    disconnect(g_game, {onGameEnd = hide, onGameStart = requestConjurerData})
    ...
    local function safeUnregister(opcode)
        if type(unregisterExtendedOpcode) == 'function' then
            ...
            if not success then
                g_logger.error(
                    "Failed to unregister opcode " .. opcode .. ": " ..
                        tostring(error))
            end
        end
    end
...
function refreshRanking()
    if g_game.isOnline() then
        ...
        if extendedOpcodesStatus.sendAvailable and
            extendedOpcodesStatus.protocolEnabled then
            g_game.sendExtendedOpcode(ExtendedIds.ConjurerRankingRequest, '')
        else
            g_logger.warning(
                "Cannot refresh ranking: Extended opcodes not available.")
            displayErrorMessage(
                "Extended opcodes not available. Please contact an administrator.")
        end
    else
...
function requestConjurerData()
    if g_game.isOnline() then
        ...
        if extendedOpcodesStatus.sendAvailable and
            ...
            g_logger.warning(
                "Function sendExtendedOpcode not available for updating conjurer data.")
            displayErrorMessage(
                "Extended opcodes not available. Please contact an administrator.")
...
function updateStatusTab(data)
    ...
    if data.nextLevelExp > 0 then
        ...
        nextRankLabel:setText('Próximo rank: ' .. data.nextRankName .. ' (' ..
                                  data.expNeeded .. ' XP)')
    else
...
function updateRankingTab(rankingData)
    ...
    for i, entry in ipairs(rankingData) do
        ...
        row:getChildById('experience'):setText(
            entry.experience .. ' (' .. (entry.rankName or 'Unknown') .. ')')

        g_logger.info("Created row " .. i .. ": " .. entry.name .. " - " ..
                          entry.experience .. ' (' ..
                          (entry.rankName or 'Unknown') .. ')')
...
function onConjurerRanking(protocol, opcode, buffer)
    g_logger.info("Received ConjurerRanking opcode with buffer length: " ..
                      buffer:len())
    ...
    if not success or not data then
        g_logger.error("Failed to decode ConjurerRanking JSON: " ..
                           tostring(data))
        return
    end
    ...
    for i, entry in ipairs(data) do
        g_logger.info("Entry " .. i .. ": " .. entry.name .. " - " ..
...
function onTestButtonClick()
    displayInfoBox('Teste de conjuração',
                   'Aqui você pode simular um efeito visual ou ganho de XP!')
end
...
function toggleConjurerRankingWindow()
    if g_game.isOnline() then
        ...
        if extendedOpcodesStatus.sendAvailable and
            ...
            g_logger.warning(
                "Extended opcodes not available for updating ranking.")
            displayErrorMessage(
                "Extended opcodes not available. Please contact an administrator.")
        end
    end
    ...
    if modules.game_conjurer and
        modules.game_conjurer.toggleConjurerRankingWindow then
