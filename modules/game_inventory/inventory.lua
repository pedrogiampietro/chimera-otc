...
function setupTopMenuButton()
    if not inventoryWindow.forceOpen then
        inventoryButton = modules.client_topmenu.addRightGameToggleButton(
                              ...
                              '/images/topbuttons/new/inventory', toggle, nil,
                              nil, true)
    end
end
...
function init()
    ...
    ProtocolGame.registerExtendedOpcode(SPECIAL_CONTAINER_OPCODE,
                                        onSpecialContainerExtendedOpcode)
    connect(g_game, {
        onGameStart = onSpecialContainerGameStart,
        onGameEnd = onSpecialContainerGameEnd
    })
    ...
    inventoryPanel = inventoryWindow:getChildById('contentsPanel'):getChildById(
                         'inventoryPanel')
    ...
    -- special container
    specialContainerSlot = inventoryWindow:recursiveGetChildById(
                               'specialContainerSlot')
    if not specialContainerSlot then
        ...
        local conditionPanel = inventoryWindow:recursiveGetChildById(
                                   'conditionPanel')
        ...
        if slot8 then
            ...
            specialContainerSlot:setImageClip({
                x = 0,
                y = 0,
                width = 34,
                height = 12
            ...
            specialContainerSlot:setTooltip(
                tr('Click to open Special Container'))
            ...
            specialContainerSlot:addAnchor(AnchorHorizontalCenter, 'parent',
    ...
    -- controls
    fightOffensiveBox = inventoryWindow:recursiveGetChildById(
                            'fightOffensiveBox')
    fightBalancedBox = inventoryWindow:recursiveGetChildById('fightBalancedBox')
    fightDefensiveBox = inventoryWindow:recursiveGetChildById(
                            'fightDefensiveBox')
...
function terminate()
    ...
    ProtocolGame.unregisterExtendedOpcode(SPECIAL_CONTAINER_OPCODE,
                                          onSpecialContainerExtendedOpcode)
    disconnect(g_game, {
        onGameStart = onSpecialContainerGameStart,
        onGameEnd = onSpecialContainerGameEnd
...
function onMinimize() conditionPanel:setOn(true) end

function onMaximize() conditionPanel:setOn(false) end

-- hooked events
function onInventoryChange(player, slot, item, oldItem)
    if InventorySlotPurse < slot then return end
    ...
    if item then
        ...
        elseif tip:find("%[mythic%]") or tip:find(" mythic") then
...
local function getSpecialContainerSlotWidget(slotIndex)
    if not specialContainerWindow then return nil end

    local slot = specialContainerSlots[slotIndex]
    if slot and not slot:isDestroyed() then return slot end

    slot = specialContainerWindow:recursiveGetChildById('specialSlot' ..
                                                            slotIndex)
    if slot then specialContainerSlots[slotIndex] = slot end
    return slot
end
...
local function updateSpecialContainerSlot(slotIndex)
    ...
    if itemData then
        ...
        if success and result then itemExists = true end
...
local function refreshSpecialContainerSlots()
    for i = 1, 5 do updateSpecialContainerSlot(i) end
end
...
local function sendSpecialContainerOpcode(opcode, payload)
    ...
    g_logger.warning(
        'Extended opcode API unavailable; unable to communicate with server for special container')
    return false
end
...
function onSpecialContainerExtendedOpcode(protocol, opcode, buffer)
    if opcode ~= SPECIAL_CONTAINER_OPCODE then return end
...
function openSpecialContainer()
    if not specialContainerWindow then
        ...
        specialContainerWindow = g_ui.createWidget('MiniWindow',
        ...
        local contentsPanel = specialContainerWindow:getChildById(
                                  'contentsPanel') or
                                  specialContainerWindow:getChildById(
                                      'miniwindowContents')
        ...
        local scrollbar = specialContainerWindow:getChildById(
                              'miniwindowScrollBar')
        if scrollbar then
    ...
    if specialContainerWindow then
        if g_game.isOnline() then requestSpecialContainerData() end
...
function onSpecialContainerClose()
    if specialContainerWindow then specialContainerWindow:hide() end
end
...
function onSpecialContainerDrop(slot, dragged, slotIndex)
    ...
    if fromSlotIndex then
        ...
        if protocolGame then
            protocolGame:sendExtendedOpcode(SPECIAL_CONTAINER_OPCODE,
                                            json.encode(data))
        end
